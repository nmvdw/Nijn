...
(* Here, we can see that the [[ . ]] is not sm on alpha *)
...
Section OrderInterpretation.
  ...
  (* This lemma cannot be proved for we need to build initial SM elements.
  Problem to think about: what assumptions or added structure we need
  to add here. *)
  (* Fixpoint sem_Ty_el
...
(*
  (* parallel to alpha in pen-and-paper stuff  *)
  Fixpoint sem_Var
           {C : con B}
           {A : ty B}
           (v : var C A)
    : sem_Con C →s sem_Ty A
    := match v with
       | Vz => fst_sm
       | Vs v => sem_Var v ∘sm snd_sm
       end.
*)
...
(*
  Definition strong_func_is_weak
    (X Y : CompatRel) (SM : X →sm Y) : X →wm Y :=
    _.
*)
